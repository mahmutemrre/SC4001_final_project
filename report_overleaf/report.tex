...
Dilated convolutions \cite{dilated} address this by inserting gaps between kernel weights, enlarging the receptive field at no additional parameter cost.
...
A learnable \texttt{[CLS]} token is prepended and positional embeddings are added.
...
MixUp consistently reduces test accuracy by approximately 0.8--1.3 percentage points across all three CNN architectures (Table~\ref{tab:main}).
...
The \textit{Shirt} class is the most challenging for all models (64--76\% accuracy depending on augmentation), reflecting its visual similarity to both \textit{T-shirt/top} and \textit{Coat}.
SE-DilatedCNN achieves the largest relative gains on the \textit{Shirt} (+1.1\,pp) and \textit{Coat} (+1.0\,pp) classes — both require discriminating subtle texture and collar features — consistent with the SE block's ability to emphasise texture-discriminative channels.
\textit{Trouser} and \textit{Bag} are the easiest classes (consistently above 97\% for all models) due to their distinctive shapes.
...
The primary confusion is between \textit{Shirt}, \textit{T-shirt/top}, and \textit{Coat} — upper-body garments with similar silhouettes.
...
The improvement is concentrated on the most confusable classes — Shirt (+1.1\,pp) and Coat (+1.0\,pp) — supporting the hypothesis that channel attention learns to up-weight texture and edge features that differentiate these garments.
